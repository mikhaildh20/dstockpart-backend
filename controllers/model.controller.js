...
export const assignModelsToLine = async (req, res) => {
    try {
        const result = await modelService.assignsModelsToLine({
            lineId: req.body.lineId || req.body.LineId,
            assignIds: req.body.assignIds || req.body.AssignIds || [],
            unassignIds: req.body.unassignIds || req.body.UnassignIds || [],
        });

        return res.status(200).json(
            success(result, "Model assignment updated successfully")
        );
    } catch (err) {
        console.error("ASSIGN MODELS ERROR:", err);

        return res.status(500).json(
            error(err.message || "Error assigning models to line")
        );
    }
};